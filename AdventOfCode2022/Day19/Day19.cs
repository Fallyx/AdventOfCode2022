...
internal class Day19
{
    ...
    public static void Task1()
    {
        ...
        foreach(string line in lines)
        {
            ...
            foreach(Match m in Regex.Matches(line, pattern))
            {
                ...
                qualityLevel += idx * HighestGeode(robotCost);
            }
        }
    ...
    private static int HighestGeode(Vector4[] robotCost)
    {
        int maxGeode = 0;
        ...
        while (queue.Count > 0)
        {
            ...
            if (currentStep.resources.X >= robotCost[3].X && currentStep.resources.Z >= robotCost[3].Z)
            {
            ...
            if (currentStep.robots.Z + 1 <= maxObsidianBots && currentStep.resources.X >= robotCost[2].X && currentStep.resources.Y >= robotCost[2].Y)
            {
                Vector4 newRobots = currentStep.robots;
                newRobots.Z++;
            ...
            if (currentStep.robots.Y + 1 <= maxClayBots && currentStep.resources.X >= robotCost[1].X)
            {
                Vector4 newRobots = currentStep.robots;
                newRobots.Y++;
            ...
            if (currentStep.robots.X + 1 <= maxOreBots && currentStep.resources.X >= robotCost[0].X)
            {
                Vector4 newRobots = currentStep.robots;
                newRobots.X++;
